...
def main():
    ...
    # d1 = Datum.from_geometry(GeometryType.PLANE)
    
    stack1 = Component("stack1")
    # Primary datums
    plane_id, axis_id = stack1.add_primary_datums(
        plane_origin=[0, 0, 0],
        plane_normal=[0, 0, -1]  # Bottom face
    )
    ...
    for n in range(3):
        # Create optical reference plane 25mm above bottom
        opt_plane_id = stack1.create_derived_datum(
            reference_id=plane_id,
            dimension_type='offset_distance',
            values=[10.0],
            target_geometry=GeometryType.PLANE,
            constraints={'direction': [0, 0, 1]},
            datum_id=f"OPT_PLANE{n}",
            name=f"OPTICAL_PLANE{n}"
        )
    
        # Add tolerance to optical plane height
        opt_plane = stack1.get_datum(opt_plane_id)
        if opt_plane.dimension:
            opt_plane.dimension.set_tolerance(Tolerance(ToleranceType.DIMENSION, 0.1, 0.1))
        
        plane_id = opt_plane_id
    ...
    opt_plane_id = stack1.create_derived_datum(
        ...
        values=[3.142/4],#deg
        target_geometry=GeometryType.PLANE,
        constraints={'reference_direction': [0, 1, 0]},
        datum_id="TILTED_PLANE",
        name="TILTED_PLANE"
    )
    
    print(stack1)
    for dn, dat in stack1.datums.items():
        print(dn,dat,dat.reference)
    
    return
    
    stack2 = Component("stack2")
    # Primary datums
    plane_id, axis_id = stack2.add_primary_datums(
        plane_origin=[0, 0, 0],
        plane_normal=[0, 0, -1]  # Bottom face
    )
    ...
    for n in range(3):
        # Create optical reference plane 25mm above bottom
        opt_plane_id = stack2.create_derived_datum(
            reference_id=plane_id,
            dimension_type='offset_distance',
            values=[10.0],
            target_geometry=GeometryType.PLANE,
            constraints={'direction': [0, 0, 1]},
            datum_id=f"OPT_PLANE2-{n}",
            name=f"OPTICAL_PLANE2-{n}"
        )
    
        # Add tolerance to optical plane height
        opt_plane = stack2.get_datum(opt_plane_id)
        if opt_plane.dimension:
            opt_plane.dimension.set_tolerance(Tolerance(ToleranceType.DIMENSION, 0.1, 0.1))
    ...
    assembly = Assembly("test_stack")
    assembly.add_component(stack1,ground=True)
